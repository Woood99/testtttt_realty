...
const EditPresent = ({ conditionModal, setModal, groupData, fetchData, specialists = [], developer = {}, defaultValues = {} }) => {
   const {
      handleSubmit,
      control,
      formState: { errors },
   ...
   const [images, setImages] = useState(defaultValues.image ? [{ id: 1, image: defaultValues.image }] : []);
   const [pdf, setPdf] = useState(defaultValues.specs ? [{ id: 1, pdf: defaultValues.specs }] : []);
   ...
   const ModalHeaderLayout = () => {
      return (
         <ModalHeader set={setModal} className="px-10 py-6 mb-8">
            <h2 className="title-2">Редактировать подарок</h2>
         </ModalHeader>
      );
   };
   ...
   const onSubmitHandler = data => {
      ...
      sendPostRequest(`/admin-api/gift/${defaultValues.id}/update`, formData, { 'Content-Type': 'multipart/form-data' }).then(res => {
         fetchData();
      });
   };
         ...
         <form>
            ...
            <div className="grid grid-cols-2 gap-2">
               ...
               <ControllerFieldSelect
                  ...
                  options={specialists}
                  name="author"
                  defaultValue={specialists.find(item => item.value === defaultValues.author_id)}
                  disabled={Boolean(specialists.find(item => item.value === defaultValues.author_id))}
               />
               <ControllerFieldInput
                  control={control}
                  datePicker={true}
                  ...
                  requiredValue
                  errors={errors}
                  defaultValue={defaultValues.start_date ? dayjs(defaultValues.start_date).format('DD.MM.YYYY') : ''}
               />
               <ControllerFieldInput
                  control={control}
                  datePicker={true}
                  ...
                  requiredValue
                  errors={errors}
                  defaultValue={defaultValues.end_date ? dayjs(defaultValues.end_date).format('DD.MM.YYYY') : ''}
               />
               <ControllerFieldInput
                  control={control}
                  beforeText="Старая цена"
                  name="old_price"
                  convertNumber
                  onlyNumber
                  maxLength={11}
                  defaultValue={defaultValues.old_price || 0}
               />
               <ControllerFieldInput
                  control={control}
                  ...
                  requiredValue
                  errors={errors}
                  defaultValue={defaultValues.new_price || 0}
               />
            </div>
            <div className="mt-6">
