...
const EditChar = () => {
   ...
   const { old_name, tabId } = qs.parse(window.location.search.substring(1));
   ...
   useEffect(() => {
      getDataRequest(`/admin-api/type/${params.typeId}/show/attribute`, { name: old_name }).then(res => {
         if (!res.data) return;
   ...
   const onSubmitHandler = data => {
      ...
      if (listSingle) {
         ...
         resData['list-single-value'] = resData['list-single-value']
            .split(',')
            .map(item => item.trim())
            .join(',');
      }
      if (listMultiple) {
         ...
         resData['list-multiple-value'] = resData['list-multiple-value']
            .split(',')
            .map(item => item.trim())
            .join(',');
      }
      if (resData.type === 'flag') {
         resData['list-single-value'] = 'Да, Нет';
      }

      sendPostRequest(`/admin-api/type/${params.typeId}/update/complex-attribute`, resData).then(res => {
         navigate(`${PrivateRoutesPath.types.show}${params.typeId}?attribute=${tabId}`);
      });
   };
         ...
         <div className="main-wrapper--title">
            <div className="container">
               <ElementTitleSubtitle>
                  Редактировать <span>характеристику жилого комплекса</span>
               </ElementTitleSubtitle>
            </div>
