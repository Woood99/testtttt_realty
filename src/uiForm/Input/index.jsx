...
const Input = props => {
   ...
   return (
      <div
         ...
         <label
            className={`${labelClassNames} ${size !== 'default' ? styles['InputSize' + size] : ''} ${isValid ? styles.InputError : ''} ${
            ...
            {mask === 'phone' && <InputMask {...inputMaskOptions} mask={phoneMask} options={{ autoComplete: 'tel' }} inputRef={inputRef} />}
            {mask === 'numbers4' && <InputMask {...inputMaskOptions} mask={numbers4Mask} inputRef={inputRef} />}
            {mask === 'hhmmssMask' && <InputMask {...inputMaskOptions} mask={hhmmssMask} inputRef={inputRef} />}
            {mask === 'hhmmMask' && <InputMask {...inputMaskOptions} mask={hhmmMask} inputRef={inputRef} />}
            {mask === 'bankCardMask' && <InputMask {...inputMaskOptions} mask={bankCardMask} inputRef={inputRef} />}
            {mask === 'expiryMask' && <InputMask {...inputMaskOptions} mask={expiryMask} inputRef={inputRef} />}
            {mask === 'cvvMask' && <InputMask {...inputMaskOptions} mask={cvvMask} inputRef={inputRef} />}
         ...
         {Boolean((selectionButtons?.options?.length && selectionButtonsOpen) || selectionButtons?.emptyText) && (
            <Tooltip mobile color="white" event="click" placement="bottom-start" value={selectionButtonsOpen} onChange={setSelectionButtonsOpen}>
               <div style={{ width: rootRef?.current?.clientWidth - 40 || 0 }}>
                  ...
                  {!selectionButtons.options.length && selectionButtons.emptyText && <div className="text-center">{selectionButtons.emptyText}</div>}
               </div>
            </Tooltip>
         )}
         {children}
...
const InputMask = ({ value, onChange, placeholder, mask, options, disabled, inputRef }) => {
   return (
      <MaskedInput
         mask={mask}
         className={styles.InputWrapper}
