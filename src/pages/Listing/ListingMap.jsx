...
import { getCurrentCitySelector, getIsDesktop } from "@/redux";
...
function ListingMap({ currentMouseEnterId, currentMouseEvent }) {
			...
			sendPostRequest(`/api/catalog/geo`, fields).then(res => {
				...
				placemarks = dataPlacemarks.map(item => {
					...
					mark.events.add("click", e => {
						if (window.innerWidth > 1212) {
							openUrl(`${RoutesPath.building}${item.id}`);
						} else {
				...
				if (item.id === currentMouseEnterId) {
					...
					if (currentMouseEvent === "enter") {
						setTimeout(() => {
							...
							if (currentMark) {
								currentMark.classList.add(
									map.getZoom() > 12.4 ? stylesPlacemark.MapPlacemarkActive : stylesPlacemark.MapCircleActive
								);
							}
						}, 150);
					}
	...
	return (
		<div className='relative w-full h-full md1:flex-grow md1:absolute md1:mt-[48px] md1:!h-[calc(var(--vh,100vh)-98px)]'>
						...
						<>
							<div className='ymap-actions top-4 right-4 ymap-actions-group !w-auto items-end'>
								{paintMore ? (
									...
									<button ref={paintBtnRef} className={`ymap-action ymap-action-btn !w-auto px-4 gap-2`}>
										{isPaintState ? (
											<>
												<IconFinger width={24} height={24} />
												<span className='text-white'>Начните рисовать</span>
												<IconCloseCircle className='text-white' />
											</>
										) : (
											<>
												Нарисовать область
					...
					{!isDesktop && (
						<ModalWrapper condition={mobileModalComplex}>
							<Modal
								...
								options={{
									modalClassNames: "",
									overlayClassNames: "_bottom _full !z-[9999999]",
									modalContentClassNames: "!px-4 max-h-[80vh]"
								}}>
