...
import { AuthRoutesPath, COOKIE_MAX_AGE, PrivateRoutesPath, ROLE_ADMIN, ROLE_BUYER, ROLE_SELLER, RoutesPath } from "@/constants";
...
import { CountdownTimerDefault, Header } from "@/components";
		...
		<>
			{!codeContinue && !currentPhone ? (
				<form onSubmit={handleSubmit(onSubmitPhone)}>
					...
					<Button size="Big" className="w-full mt-4" isLoading={isLoadingSubmit}>
						Получить код
					</Button>
				</form>
			) : (
				<form onSubmit={handleSubmit(onSubmitCode)}>
					<div className="mb-6">
						...
						<div className="mt-6 flex items-center gap-4 md3:flex-col md3:items-start md3:gap-1.5">
							<p>
					...
					>
						{!visibleAdditional ? (
							<>
								...
								<CountdownTimerDefault initialTime={60} onComplete={() => setVisibleAdditional(true)} />
								&nbsp;сек.
							</>
						) : (
							"Получить новый код"
	...
	return (
		<Modal
			options={{
				...
				modalContentClassNames: "!px-10 md1:!px-6 !pt-[115px]"
			}}
			condition={condition}
			set={set}
		>
