...
import { getApartment, getDataRequest, getSpecialists } from "@/api";

import { useCallingPartner, useNavigateToChat, useQueryParams } from "@/hooks";

import { getApartmentTitle, getFirstElementArray, isBuyer, isEmptyArrObj, priceByDiscountApartment } from "@/helpers";
...
export const useApartment = id => {
	...
	const goToChatCall = async () => {
		await callingPartner({ building_id: apartmentData.building_id, organization_id: apartmentData.developer?.id });
	};
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			const cashbackValue =
				((apartRes.bd_price || apartRes.price) / 100) * ((apartRes.cashback || 0) + (apartRes.buildingCashback?.value || 0));
