...
import { setCurrentPage, setFiltersFeed, setType, setValueFeed } from '../../redux/slices/feedSlice';
...
export const useFeed = options => {
   const { searchParamsTab, fetch = () => {}, feedType } = options;
   ...
   const { tags: tagsDataAll, isLoading: isLoadingTags } = useGetTags(
      { type: 'tags', assigned: feedSelector.type },
      JSON.stringify({
         type: feedSelector.type,
         city: feedSelector.values.city.value,
      })
   );
   ...
   const fetchData = useCallback(
      debounce(state => {
         ...
         setFilterCount(getCountOfSelectedFilterObj(fetchParams, ['type', 'page'], { city: currentCity.name }));

         fetch(fetchParams, { setTotalPages, setDataCards }).then(() => {
   ...
   useEffect(() => {
      setTags(
         tagsDataAll
            .filter(item => item.city === feedSelector.values.city.label)
            .map(item => ({
               value: item.id,
               label: item.name,
            }))
