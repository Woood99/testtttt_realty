...
const dataNearest = {
   ...
   minWidth: 215,
   icon: <IconLocation width={15} height={15} className="!fill-primary400" />,
};
...
const LayoutButtonsInfrastructure = ({ options }) => {
   ...
   return (
      <HorizontalScrollMouse widthScreen={1222} className={` md1:!w-full flex items-center gap-2 ${window.innerWidth > 1222 ? 'top-4' : 'bottom-4'}`}>
         {dataInfrastructure.map(item => {
            const isActive = !isLoading && placemarks.find(currentItem => currentItem.type === item.type).items.length && currentType === item.type;
