...
const QuestionToChat = ({ classNames, building_id, organization_id, isSpecialist, specialist_id }) => {
	...
	return (
		<div className={cn(classNames)}>
			<h2 className='title-2 mb-2'>Трудно сделать выбор?</h2>
			<p className='mb-6'>Задайте вопрос менеджеру, он поможет с выбором квартиры и проконсультирует по всем вопросам</p>
			...
			<div className='flex gap-2 flex-wrap'>
				{CHAT_TAGS.map((item, index) => (
					<Tag
						...
						hoverEnable
						className={cn(value === item.text && "bg-primary200")}>
			...
			<div className='mt-4 relative flex items-center gap-2'>
				...
				<button
					...
					<IconSend width={26} height={26} className='fill-[#fff]' />
				</button>
			</div>
		</div>
	);
