...
const customFullscreen = (player, setIsFullscreen) => {
   const fullscreen = player.controlBar.getChild('fullscreenToggle');
   if (!fullscreen) return;
   fullscreen.dispose();

   const CustomFullscreenBtn = ({ onClick }) => {
      return (
   ...
   fullscreenBtnContainer.className = 'vjs-control vjs-button vjs-custom-fullscreen';

   ReactDOM.createRoot(fullscreenBtnContainer).render(
      <CustomFullscreenBtn
         onClick={() => {
   ...
   player.controlBar.el().insertBefore(fullscreenBtnContainer, player.controlBar.getChild('progressControl').el());
};
